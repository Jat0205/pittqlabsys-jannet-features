...
init:
  ...
  count_time = 30
  sequence_duration = 1400 ' since Par_9 is given in ns and IO_Sleep accepts params in 10ns, we divide by 10
  ...
  DIGOUT(21, 0)
  DIGOUT(16, 0)
  delay = (sequence_duration + 150 + 12) * 3 ' * 10 since the variables are in 10 ns and /(10/3) as the value has to be in number or clock ticks which is 10/3 for T11 processor
  Processdelay = delay
  i = 0
  DO
...
event:
  ...
  '  iteration_number = 0
  'ENDIF
  IF (iteration_number = 1000) THEN 'if we did all of our iterations for a given sequence, then go back to iteration 0
    iteration_number = 0
    j = j+1
  ENDIF
